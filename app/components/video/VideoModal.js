...
function VideoModal({ src, videoTitle, theme = '', children, ...props }) {
	...
	return (
		<>
			...
			{
				children ? <div onClick={() => setModalOpen(true)}>{children}</div> : <Button className={classNames('btn', theme === 'white' ? 'white' : 'textbtn')} direction='left' icon='play_circle_big' onClick={() => setModalOpen(true)} {...props}>Watch video</Button>
			}
